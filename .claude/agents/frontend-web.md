...
- **UX/UI — leer el diseño más allá de la copia y el layout general (retroalimentación explícita del usuario, 2026-07-15, no repetir este hallazgo)**: cuando implementes contra un Figma de referencia, presta atención deliberada a cómo el diseño comunica **estructura y affordance**, no solo qué texto/dato va en cada sitio. En concreto:
  - **Elementos clicables deben distinguirse visualmente de texto estático** (un dato en negrilla no comunica "esto es un link/acción" — Figma suele usar un color de acento, ej. azul, para texto/íconos interactivos). Si un elemento del mock es clicable (navega, abre un panel, dispara una acción), su estilo en tu implementación debe distinguirse de un dato de solo lectura con el mismo peso tipográfico — no te limites a copiar el texto y el tamaño de fuente.
  - **Estructura visual real, no solo aproximada**: si el mock muestra una tabla/grid con celdas delimitadas (bordes, separadores), badges de conteo junto a un título de sección, o una barra de filtros en una posición específica respecto a otros controles (ej. filtros a la izquierda del encabezado, selector de sub-vista a la derecha), replica esa estructura — una lista de `<div>` apilados con la misma información pero sin la grilla/badges/posición real no cumple el mock, aunque los datos sean correctos.
  - **Sub-vistas de una misma feature comparten cabecera/controles** cuando así lo muestra el Figma (ej. una barra de filtros y un selector de vista compartidos entre 3 pestañas de una "Matriz") — no dupliques ni omitas ese layout compartido en cada sub-vista por separado.
  - **Los `Select`/filtros del mock casi siempre existen para condicionar lo que se ve, no solo para navegar** — si Figma muestra selects de Módulo/Categoría/Estado junto a una tabla, confirma si deben filtrar esa tabla (lo más probable) antes de asumir que son solo decorativos o que son el único mecanismo de selección.
  - Antes de dar por cerrada una pantalla construida contra Figma, compara tu propia captura (`yarn dev` + navegador o screenshot) lado a lado con `get_screenshot` del frame — no te bases solo en haber leído el `get_design_context` textual.
