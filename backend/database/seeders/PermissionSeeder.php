...
class PermissionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        $permissions = [
            ...
            // Un residuo APROBADO no retrocede: solo EcoLink puede retirarlo de
            // circulación (Aprobado <-> Suspendido), conservando la trazabilidad
            // de solicitudes, programaciones y certificados que ya cuelgan de él.
            ['code' => 'wastes.suspend', 'name' => 'Suspender/reactivar residuos aprobados (solo EcoLink)', 'module' => 'wastes', 'action' => 'suspend'],
            ...
            // Segundo par de ojos dentro del Gestor (2026-08-14): quien asigna
            // el tratamiento y quien da el visto bueno final son personas
            // distintas, separadas por PERMISO -- no por rol, para que cada
            // organización se lo asigne al cargo que corresponda.
            ['code' => 'treatment_approvals.approve', 'name' => 'Aprobación final del residuo (Clasificado -> Aprobado)', 'module' => 'treatment_approvals', 'action' => 'approve'],
            // Fase 2 (2026-08-15): registrar la evaluación de un Gestor DE
            // REFERENCIA, que la resolvió en su propia plataforma y no tiene
            // usuarios aquí. NO entra en `Permission::GESTOR_ONLY_CODES` a
            // propósito: lo ejercen EcoLink y los Subgestores, que no tratan
            // residuos ellos mismos.
            ['code' => 'treatment_approvals.assign_delegated', 'name' => 'Asignar tratamiento en nombre de un Gestor de referencia', 'module' => 'treatment_approvals', 'action' => 'assign_delegated'],
            ['code' => 'subgestor_gestor_relationships.create', 'name' => 'Vincular un Gestor a mi organización Subgestora', 'module' => 'subgestor_gestor_relationships', 'action' => 'create'],
            ['code' => 'subgestor_gestor_relationships.revoke', 'name' => 'Revocar el vínculo con un Gestor', 'module' => 'subgestor_gestor_relationships', 'action' => 'revoke'],
            ...
            // Cadena Generador -> Subgestor -> Gestor en Declaración de
            // Residuos (confirmado por stakeholders reales, 2026-08-09) --
            // mismo criterio granular EXACTO que `gestor_carrier_authorizations.*`
            // (`.read` ambos lados accesibles; `.create` solo el Subgestor
            // dueño registra; `.revoke` separado, mismo criterio que
            // `wastes.activate`/`.deactivate`). Ninguno es `is_critical`.
            ['code' => 'generator_subgestor_relationships.read', 'name' => 'Consultar relaciones Generador-Subgestor', 'module' => 'generator_subgestor_relationships', 'action' => 'read'],
            ['code' => 'generator_subgestor_relationships.create', 'name' => 'Registrar Generadores clientes', 'module' => 'generator_subgestor_relationships', 'action' => 'create'],
            ['code' => 'generator_subgestor_relationships.revoke', 'name' => 'Revocar relaciones Generador-Subgestor', 'module' => 'generator_subgestor_relationships', 'action' => 'revoke'],

            // Carga Masiva de Generadores (CSV) por Subgestor/Gestor
            // (confirmado por el usuario, 2026-08-11) -- vínculo comercial
            // DIRECTO Generador<->Gestor, mismo criterio granular que
            // `generator_subgestor_relationships.*`. `GeneratorBulkImportController`
            // reutiliza `.create` de este módulo (Gestor) o del de arriba
            // (Subgestor) según la capacidad de la organización actora --
            // sin permiso nuevo dedicado a "carga masiva" en sí.
            ['code' => 'generator_gestor_relationships.read', 'name' => 'Consultar relaciones Generador-Gestor', 'module' => 'generator_gestor_relationships', 'action' => 'read'],
            ['code' => 'subgestor_gestor_relationships.read', 'name' => 'Consultar relaciones Subgestor-Gestor', 'module' => 'subgestor_gestor_relationships', 'action' => 'read'],
            ['code' => 'generator_gestor_relationships.create', 'name' => 'Registrar Generadores clientes directos', 'module' => 'generator_gestor_relationships', 'action' => 'create'],
            ['code' => 'generator_gestor_relationships.revoke', 'name' => 'Revocar relaciones Generador-Gestor', 'module' => 'generator_gestor_relationships', 'action' => 'revoke'],
        ];
